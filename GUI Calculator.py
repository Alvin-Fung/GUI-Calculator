...
class StandardPage(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        Operations.window_result = Operations(
            self, height=2, width=16, font=("Roboto Mono", 24))
        Operations.window_result.grid(columnspan=5)

        btn_1 = CalculatorButton(
            self, "1", lambda: Operations.window_result.add_to_operation(1))
        btn_1.grid(row=2, column=1)

        btn_2 = CalculatorButton(
            self, "2", lambda: Operations.window_result.add_to_operation(2))
        btn_2.grid(row=2, column=2)

        btn_3 = CalculatorButton(
            self, "3", lambda: Operations.window_result.add_to_operation(3))
        btn_3.grid(row=2, column=3)

        btn_4 = CalculatorButton(
            self, "4", lambda: Operations.window_result.add_to_operation(4))
        btn_4.grid(row=3, column=1)

        btn_5 = CalculatorButton(
            self, "5", lambda: Operations.window_result.add_to_operation(5))
        btn_5.grid(row=3, column=2)

        btn_6 = CalculatorButton(
            self, "6", lambda: Operations.window_result.add_to_operation(6))
        btn_6.grid(row=3, column=3)

        btn_7 = CalculatorButton(
            self, "7", lambda: Operations.window_result.add_to_operation(7))
        btn_7.grid(row=4, column=1)

        btn_8 = CalculatorButton(
            self, "8", lambda: Operations.window_result.add_to_operation(8))
        btn_8.grid(row=4, column=2)

        btn_9 = CalculatorButton(
            self, "9", lambda: Operations.window_result.add_to_operation(9))
        btn_9.grid(row=4, column=3)

        btn_0 = CalculatorButton(
            self, "0", lambda: Operations.window_result.add_to_operation(0))
        btn_0.grid(row=5, column=2)

        add = CalculatorButton(
            self, "+", lambda: Operations.window_result.add_to_operation("+"))
        ...
        clear = CalculatorButton(
            self, "C", Operations.window_result.clear_operation)
        clear.grid(row=6, column=2, columnspan=2)

        equals = CalculatorButton(
            self, "=", Operations.window_result.evaluate_operation)
