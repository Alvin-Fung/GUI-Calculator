...
class StandardPage(tk.Frame):
     def __init__(self, parent):
        ...
        btn_1 = CalculatorButton(self, "1", lambda: Operations.add_to_operation(1))
        btn_1.grid(row = 2, column  = 1,)

        btn_2 = CalculatorButton(self, "2", lambda: Operations.add_to_operation(2))
        btn_2.grid(row = 2, column = 2)

        btn_3 = CalculatorButton(self, "3", lambda: Operations.add_to_operation(3))
        btn_3.grid(row = 2, column = 3)

        btn_4 = CalculatorButton(self, "4", lambda: Operations.add_to_operation(4))
        btn_4.grid(row = 3, column = 1)

        btn_5 = CalculatorButton(self, "5", lambda: Operations.add_to_operation(5))
        btn_5.grid(row = 3, column = 2)

        btn_6 = CalculatorButton(self, "6", lambda: Operations.add_to_operation(6))
        btn_6.grid(row = 3, column = 3)

        btn_7 = CalculatorButton(self, "7", lambda: Operations.add_to_operation(7))
        btn_7.grid(row = 4, column = 1)

        btn_8 = CalculatorButton(self, "8", lambda: Operations.add_to_operation(8))
        btn_8.grid(row = 4, column = 2)

        btn_9 = CalculatorButton(self, "9", lambda: Operations.add_to_operation(9))
        btn_9.grid(row = 4, column = 3)

        btn_0 = CalculatorButton(self, "0", lambda: Operations.add_to_operation(0))
        btn_0.grid(row = 5, column = 2)

        add = CalculatorButton(self, "+", lambda: Operations.add_to_operation("+"))
        add.grid(row = 2, column = 4)

        minus = CalculatorButton(self, "-", lambda: Operations.add_to_operation("-"))
        minus.grid(row = 3, column = 4)

        mul = CalculatorButton(self, "*", lambda: Operations.add_to_operation("*"))
        mul.grid(row = 4, column = 4)

        div = CalculatorButton(self, "/", lambda: Operations.add_to_operation("/"))
        div.grid(row = 5, column = 4)

        l_bracket = CalculatorButton(self, "(", lambda: Operations.add_to_operation("("))
        l_bracket.grid(row = 5, column = 1)

        r_bracket = CalculatorButton(self, ")", lambda: Operations.add_to_operation(")"))
        r_bracket.grid(row = 5, column = 3)

        decimal = CalculatorButton(self,".", lambda: Operations.add_to_operation("."))
        decimal.grid(row = 6, column = 1, columnspan = 2)

        clear = CalculatorButton(self, "C", Operations.clear_operation)
        clear.grid(row = 6, column = 2, columnspan = 2)

        equals = CalculatorButton(self, "=", Operations.evaluate_operation)
